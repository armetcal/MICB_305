...
I recommend the following: **a) Run each chunk one command at a time (use Ctrl+Enter or Cmd+Return)** and make sure you understand what's happening. b) If using this as a refernce: press the 'Outline' button at the top right of this screen (near the Run button) to see the sections of this script, click to go to that section
...
```{r}
# Find patterns
str_detect("hello",pattern='h')

# Modify string
str_replace("hello", pattern='h', replacement = 'j')
...
str_sub("hello",-5,-1) # Negative numbers count from the end
```

Before looking at str_split, let's look at vectors vs lists.

```{r}
# Vectors - analogous to matrices
a = c(1,2,3)
b = c(4,5,6)
b[2] # Second value in vector

c(4,'b',6) # Converts everything to the same type
c(a,b) # 'Flattens' a and b
c(v1 = a, v2 = b) # Can add names to every value

# Lists - analogous to data frames
list(4,'b',6)
list(a,b)
my_list = list(v1 = a, v2 = b)
my_list
my_list$v2
my_list[[2]] # Second value in list
my_list[[2]][2] # 2nd value in 2nd value of list

# Lists are analogous to directories
folder = list(file_1 = 'TACGTCAG', 
              file_2 = 'GCCCATGAT')
root_directory = list(folder_1 = folder)
root_directory
```

Now we can look at str_split:

```{r}
# Split strings
split = str_split("hello",pattern = 'l')
split
...
```{r}
# Join strings
paste('adds','space') # sep is ' ' by default
paste0('no','space')
...
These commands also work on a *vector* of strings - not list(), which is different, but c().
```{r}
...
table(A %in% B) # Is each value of A equal to any value of B?
...
# Print the tables for our reference
df1;df2
```
...
```{r same_names}
full_join(df1,df1) # Uses all columns that share a name to join, therefore no difference
full_join(df1,df1, by = 'ID') # Treats any other column as separate data EVEN IF the data are identical. Adds suffixes to denote origin.
```

What if the columns you're using to join have different values?

```{r diff_values}
df1_mutated = df1
df1_mutated$Age[1] = 99 # Change one value so not identical
full_join(df1,df1_mutated) # Now sample 1 has two rows. Always inspect your outputs!!!!! Common error with big datasets.
```

Base R version of 'join' commands

```{r merge}
merge(df1,df2) # Default is inner join
merge(df1,df2, all=TRUE) # Full join
merge(df1,df2, all.x=TRUE) # Left join
merge(df1,df2, all.y=TRUE) # Right join
merge(df1,df2, by='row.names') # Merge by rownames - none of the columns are used
```
...
calculate_diversity(faulty_data)
...
# Even if you have a default value, if you write '2' without specifying which variable it is for, R will assume it's for the first variable
new_function = function(x=0,y,z=4) x * y + z
new_function(2)
new_function(y=2)
...
new_function(1,2,3) # You can overwrite default values
```
...
download.file('https://raw.githubusercontent.com/armetcal/MICB_305/refs/heads/main/R%20Scripts/mystery_script.R', "mystery_script.R")

in_environment = objects() # List objects in current environment
source('mystery_script.R')
in_environment_after = objects()

# What new objects were created?
setdiff(in_environment_after, in_environment) 
...
named_vector = c(a=10,b=100,c=1000)
named_vector
...
```{r}
df_of_shapes = data.frame(colour = c(rep('blue',3),
                                 rep('red',3)),
...
# Using group_modify
# It works almost the same in this case, but group_modify is far more flexible
