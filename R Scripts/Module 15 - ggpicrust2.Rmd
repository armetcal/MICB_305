...
Let's see which features are essentially the same (they'll be highly correlated).
Keep this in mind when writing up your results.

```{r identical features}
sig_features = daa_annotated_results_df %>% 
  filter(p_adjust < 5e-11) %>% 
  pull('feature') %>% unique()
...
stats = ko_relab %>% t() %>%  # switch rows and columns
  ...
  cor(method = 'pearson') #Spearman tests between every feature pair
...
Unfortunately, the size of the text can't be increased, even with theme() commands.
